...
int main(int argc, char* argv[])
{
	...
	int* chunk;
	FILE* file = NULL;
	double time_taken = 0.0; // Inisialisasi 0 agar perhitungannya aman
	double time_sequential = 0.0; // TAMBAHAN: Deklarasi variabel untuk waktu sekuensial
	MPI_Status status;
	...
	if (rank_of_process == 0) {
		...
		// --- TAMBAHAN: PENGUKURAN WAKTU SEKUENSIAL ---
		// Membuat salinan data agar data asli tidak terpengaruh untuk proses paralel nanti
		int* data_copy = (int*)malloc(number_of_elements * sizeof(int));
		for (int i = 0; i < number_of_elements; i++) {
			data_copy[i] = data[i];
		}

		double start_seq = MPI_Wtime(); // Memulai timer sekuensial
		quicksort(data_copy, 0, number_of_elements); // Menjalankan sorting pada 1 process
		double end_seq = MPI_Wtime();   // Menghentikan timer sekuensial
		
		time_sequential = end_seq - start_seq; // Menyimpan selisih waktu
		free(data_copy); // Membebaskan memori salinan
		// ----------------------------------------------
	...
	if (rank_of_process == 0) {
		...
		// --- TAMBAHAN: MODIFIKASI TAMPILAN PERBANDINGAN WAKTU ---
		printf("\n\n=== PERBANDINGAN WAKTU ===");
		printf("\nWaktu Sekuensial (Tanpa MPI)   : %f secs", time_sequential);
		printf("\nWaktu Paralel (%d Process MPI) : %f secs\n", number_of_process, time_taken);
		printf("==========================\n");
		// --------------------------------------------------------
	}
